import pygame
from data.code.support import import_csv, import_tiles
from data.code.settings import tile_size, screen_height, screen_width
from data.code.enemy import Enemy
from data.code.tiles import Tile, StaticTile, AnimeTile, Coin, StumpTile
from data.code.decoration import Sky, Void, Clouds
from data.code.player import Player
from data.code.particles import ParticleEffect
from data.code.game_data import levels

class Level:
    def __init__(self,current_level,surface,create_overworld,create_end_of_lvl,start_time,get_bacon,get_egg,change_health):
        #screen setup
    ...
    def create_tiles(self,layout,type):
        ...
        path = f'./data/PixelArt/{type}/{type}_tiles.png'
            ...
            for col_index,val in enumerate(row):
                if val != '-1':
                    ...
                    if type == 'coins':
                        ...
                        tile = tile_list[int(val)]
                        if int(val) == 0:
                            sprite = Coin(tile_size,x,y,'./data/PixelArt/coins/bacon','bacon')
                        if int(val) == 1:
                            sprite = Coin(tile_size,x,y,'./data/PixelArt/coins/egg','egg')
                    if type == 'enemy':
                        tile_list = import_tiles(path)
                        tile = tile_list[int(val)]
    ...
    def player_setup(self,layout,change_health):
        for row_index,row in enumerate(layout):
            for col_index,val in enumerate(row):
                ...
                if val == '1':
                    sprite = AnimeTile(tile_size,x,y,"./data/PixelArt/goal")
    ...
    def enemy_collision(self):
        ...
        if enemy_collisions:
            for enemy in enemy_collisions:
                ...
                #Finding the top half of the enemy
                if enemy_top < player_bottom < enemy_center and self.player.sprite.direction.y >= 0:
                    self.player.sprite.direction.y = -5
                    death_sprite = ParticleEffect(enemy.rect.midtop,'enemy_death')
